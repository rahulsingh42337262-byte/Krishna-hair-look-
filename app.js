...
$("bookingForm").onsubmit=async e=>{
 ...
 try{
   ...
   // Secure Telegram owner notification via Supabase Edge Function.
   // The bot token is NEVER stored in the browser.
   if(sb){
     try{
       const {error:tgError}=await sb.functions.invoke("send-telegram-notification",{
         body:{booking:{
           id:booking.id,
     ...
     }catch(tgErr){ console.warn("Telegram notification error:",tgErr); }
   }
   try{
