...
This version is prepared to send a Telegram notification after a booking is saved in Supabase.

**Important:** Never put the Telegram bot token in `app.js`, `config.js`, GitHub, or the browser.

Supabase Edge Function: `send-telegram-notification`

Required Supabase secret:
- `TELEGRAM_BOT_TOKEN` = the NEW token generated by BotFather after revoking the exposed token.

Optional secret:
- `TELEGRAM_CHAT_ID` = `8670536683` (already used as the fallback in the function).

The website calls the Edge Function only when Supabase is configured. If the function is not configured yet, the booking still saves; Telegram notification will not be sent until the function is deployed and the secret is added.
